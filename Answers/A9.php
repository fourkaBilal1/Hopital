<?php
  ...
  La base de donnees n'existe pas !
	...
	//$statement = $db->query("SELECT * FROM hdb.patient where NumPat in(SELECT NumPat FROM hdb.acte WHERE NumPat not in (SELECT NumPat FROM hdb.hospitalisation))");
	if (!($statement = $db->query("SELECT * FROM hdb.patient where NumPat in(SELECT NumPat FROM hdb.acte WHERE NumPat not in (SELECT NumPat FROM hdb.hospitalisation))"))) {
	  echo("<div class=\"alert alert-danger\" role=\"alert\">
  <span class=\"glyphicon glyphicon-exclamation-sign\" aria-hidden=\"true\"></span>
  <span class=\"sr-only\">Erreur:</span>
  ??
