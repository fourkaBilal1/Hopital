<?php
  ...
  La base de donnees n'existe pas !
	...
	/*$statement = $db->query("SELECT * FROM hdb.patient WHERE hdb.patient.NumPat in (
    SELECT NumPat FROM hdb.hospitalisation where DateSortie-Date(IFNull(DateEntree,CURDATE())) > 3
                                                      ) 
AND hdb.patient.Mutuelle <> 'MUT'
");*/
	if (!($statement = $db->query("SELECT * FROM hdb.patient WHERE hdb.patient.NumPat in (
    SELECT NumPat FROM hdb.hospitalisation where DateSortie-Date(IFNull(DateEntree,CURDATE())) > 3
                                                      ) 
AND hdb.patient.Mutuelle <> 'MUT'
"))) {
